...
REGION_BBOXES = {
    ...
    "Southeast Asia": {
        ...
        "countries_included": [
            ...
            "Papua New Guinea", "Parts of southern China"
        ],
    },
    "Latin America": {
        ...
        "countries_included": [
            ...
            "Chile", "Argentina", "Uruguay", "Cuba", "Jamaica", "Haiti", 
...
def create_enhanced_model_metadata():
    ...
    models_data.append({
        ...
        "framework": "ONNX",
        "input_shape": 17,
        "input_channels": ["B02", "B03", "B04", "B05", "B06", "B07", "B08", "B11", "B12",
                          "NDVI", "NDRE", "EVI", "VV", "VH", "DEM", "lon", "lat"],
        "output_shape": 25,
        "output_classes": [
            "Background", "Other_large_scale_cropland", "Pasture", "Mining",
            "Other_small_scale_cropland", "Roads", "Forest", "Plantation_forest",
            "Coffee", "Build_up", "Water", "Oil_palm", "Rubber", "Cacao",
...
def save_enhanced_metadata(df_models, output_dir="model_metadata"):
    ...
    df_export['input_channels'] = df_export['input_channels'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
    df_export['output_classes'] = df_export['output_classes'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
    df_export['countries_covered'] = df_export['countries_covered'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
