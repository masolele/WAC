...
CLD_PRJ_DIST = 1
BUFFER = 50
bandsused =  ['B1', 'B2', 'B3', 'B4','B5', 'B6', 'B7', 
                  'B8', 'B8A', 'B11', 'B12']
...
def calculateMaskedPercent(image, band, geom):
    maskedPixelCount = image.select(band).reduceRegion(
        reducer= ee.Reducer.count(),
        geometry= geom,
        scale= 30,
        maxPixels= 1e13).get(band)
    
    totalPixelCount  = image.unmask().select(band).reduceRegion(
        reducer= ee.Reducer.count(),
        geometry= geom,
        scale= 30,
        maxPixels= 1e13).get(band)

    cloud_cover_roi = ee.Number(1).subtract(ee.Number(maskedPixelCount).divide(totalPixelCount)).multiply(100)
...
#LOAD THE MODEL
model1 = load_model("Lag_time/model/best_weights_att_unet_lagtime_5_Fused3_2023_totalLoss6V1_without_loss_sentAfrica5.hdf5", compile=False)
...
def pad_image(image, patch_size, padding_mode='reflect'):
    ...
    padded_image = np.pad(image, ((pad_h, pad_h), (pad_w, pad_w), (0, 0)), mode=padding_mode)
...
def tile(root_directory, featlist, year=0, filename1=0, buffer_size=0, off1=0, off2=0):
  ...
  for f in featlist:
    ...
    sent2023 = create_s1_composite(feat.bounds().buffer(buffer_size).bounds(), START_DATE, END_DATE, 'median') 
    sent2023_2 = create_s1_composite(feat.bounds().buffer(buffer_size).bounds(), START_DATE2, END_DATE2, 'median') 

    Longitude = ee.Image.pixelLonLat().select('longitude').clip(feat.bounds().buffer(buffer_size).bounds())
    Latitude = ee.Image.pixelLonLat().select('latitude').clip(feat.bounds().buffer(buffer_size).bounds())#.buffer(250).bounds()
    Elevation = ee.ImageCollection('COPERNICUS/DEM/GLO30').select('DEM').mosaic().clip(feat.bounds().buffer(buffer_size).bounds())
    ...
    csPlus = ee.ImageCollection('GOOGLE/CLOUD_SCORE_PLUS/V1/S2_HARMONIZED').filterDate('2024-01-01','2024-12-30');
    ...
    basemap23 = s2.filterBounds(SA).filterDate('2024-01-01','2024-12-30').linkCollection(csPlus, [QA_BAND])\
    .map(clearMask).median().clip(SA).select(BANDS)

    #Add forest loss from hansen forest loss data
    loss22 = ee.Image('UMD/hansen/global_forest_change_2023_v1_11').clip(feat.bounds().buffer(buffer_size).bounds());
    treeLoss = loss22.select(['loss'])
    
    sent = sent2023_2


    image =basemap23.addBands(sent).addBands(Elevation).addBands(Longitude).addBands(Latitude).addBands(treeLoss).int16()
    ...
    if os.path.exists(filename2):
        ...
        try: 
            x_img = io.imread(root_directory+"planet2_"+year+"/"+str(disS)+'.tif') #Read each image
            ...
            if x_img.shape[2]==15:
                ...
                try:
                    print(x_img.shape)
                    loss = x_img[:,:,14].astype("uint8") #index 8 is out of bounds for axis 2 with size 7
                    ...
                    predictions_smooth2 = predict_img_with_smooth_windowing(
                        ...
                        pred_func=(
                            ...
                            lambda img_batch_subdiv: model1.predict_on_batch(img_batch_subdiv)
                        )
                    )
                    stopp = datetime.now()
                    #Execution time 
                    execution_time_prediction = stopp-startp
                    print("prediction execution time is: ", execution_time_prediction)
                    ...
                    conf = np.subtract(np.max(predictions_smooth, axis=2), np.partition(predictions_smooth, -2, axis=2)[:,:, -2])
                    ...
                    probCocoa = np.reshape(probCocoa, (probCocoa.shape[0],probCocoa.shape[1],1))
                    probCocoa = np.round(probCocoa*100).astype(np.uint8)

                    #prob for cocoa
                    probCoffee = predictions_smooth[:,:,8]
                    probCoffee = np.reshape(probCoffee, (probCoffee.shape[0],probCoffee.shape[1],1))
                    ...
                    slice_ = (slice(off1,a.height-off2),slice(off1,a.width-off2))
                    window_slice = windows.Window.from_slices(*slice_)                
                    #print(a.profile)
                    predimg = a.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                      #profile = src.profile.copy()
                      #print(src)
                      #print(src.profile)
    
                      aff = rasterio.windows.transform(window_slice, src.transform)
                    ...
                    with rasterio.open(Pred+
                            str(disS)+"_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
                    ...
                    slice_ = (slice(off1,b.height-off2),slice(off1,b.width-off2))
                    window_slice = windows.Window.from_slices(*slice_)
    
                    #print(a.profile)
                    predFimg = b.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                      #profile = src.profile.copy()
                      #print(src)
                      #print(src.profile)
                      ...
                      profile.update({
                              ...
                              'transform': aff})
                    #print('georeferencing now')
                    with rasterio.open(PredFull_dir+
                            str(disS)+"_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
                    ...
                    slice_ = (slice(off1,c.height-off2),slice(off1,c.width-off2))
                    window_slice = windows.Window.from_slices(*slice_)
                    #print(a.profile)
                    confimg = c.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                     ...
                     # window_slice = windows.Window.from_slices(*slice_)
                      ...
                      profile.update({
                              ...
                              'transform': aff})  
                    #print('georeferencing now')
                    with rasterio.open(conf_dir+
                            str(disS)+"conf_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
                    ...
                    slice_ = (slice(off1,d.height-off2),slice(off1,d.width-off2))
                    window_slice = windows.Window.from_slices(*slice_)                
                    #print(a.profile)
                    probimg = d.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                      #profile = src.profile.copy()
                      #print(src)
                      #print(src.profile)
                      ...
                      profile.update({
                              ...
                              'transform': aff})  
                    #print('georeferencing now')
                    with rasterio.open(prob_dir+
                            str(disS)+"prob_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
                    ...
                    slice_ = (slice(off1,e.height-off2),slice(off1,e.width-off2))
                    window_slice = windows.Window.from_slices(*slice_)                
                    #print(a.profile)
                    probimgCocoa = e.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                      #profile = src.profile.copy()
                      #print(src)
                      #print(src.profile)
                      ...
                      profile.update({
                              ...
                              'transform': aff})  
                    #print('georeferencing now')
                    with rasterio.open(probCocoa_dir+
                            str(disS)+"prob_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
                    ...
                    window_slice = windows.Window.from_slices(*slice_)                
                    #print(a.profile)
                    probimgCoffee = e.read(1, out_dtype='uint8', window=window_slice) # read the entire array
                    with rasterio.open(filename1, count=1, dtype='uint8') as src:
                      #profile = src.profile.copy()
                      #print(src)
                      #print(src.profile)
                      ...
                      profile.update({
                              ...
                              'transform': aff})  
                    #print('georeferencing now')
                    with rasterio.open(probCoffee_dir+
                            str(disS)+"prob_georef.tif", 'w', **profile) as dst:
                            #print('Its all well')
...
tile(root_directory, featlist=featlist, year=2024, buffer_size= 1, off1=0, off2=0) #50, 11, 10

stop2 = datetime.now()
#Execution time 
execution_time_download = stop2-start2
print("Data download, prediction and georeferencing execution time is: ", execution_time_download)
