...
def conv_blockD(x, filter_size, size, dropout, batch_norm=False, dilation_rate=1):
    ...
    """
    conv = layers.Conv2D(
        size, (filter_size, filter_size), padding="same", dilation_rate=dilation_rate
    )(x)
    if batch_norm:
        conv = layers.BatchNormalization(axis=3)(conv)
    conv = layers.Activation("relu")(conv)
...
def attention_block(x, gating, inter_shape):
    ...
    upsample_psi = layers.UpSampling2D(
        size=(shape_x[1] // sigmoid_xg.shape[1], shape_x[2] // sigmoid_xg.shape[2])
    )(sigmoid_xg)
...
def Attention_UNetFusion3I_Sentinel2_Binary(
    ...
    up_16 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE),
        data_format="channels_last",
        interpolation="bilinear",
    )(conv_8)
    up_16 = layers.concatenate([up_16, att_16], axis=3)
    up_conv_16 = conv_block(
        up_16, FILTER_SIZE, 4 * FILTER_NUM, dropout_rate, batch_norm
    )
    # UpRes 7
    gating_32 = gating_signal(up_conv_16, 2 * FILTER_NUM, batch_norm)
    att_32 = attention_block(conv_32, gating_32, 2 * FILTER_NUM)
    up_32 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE),
        data_format="channels_last",
        interpolation="bilinear",
    )(up_conv_16)
    up_32 = layers.concatenate([up_32, att_32], axis=3)
    up_conv_32 = conv_block(
        up_32, FILTER_SIZE, 2 * FILTER_NUM, dropout_rate, batch_norm
    )
    # UpRes 8
    gating_128 = gating_signal(up_conv_32, FILTER_NUM, batch_norm)
    att_128 = attention_block(conv_128, gating_128, FILTER_NUM)
    up_128 = layers.UpSampling2D(
        size=(UP_SAMP_SIZE, UP_SAMP_SIZE),
        data_format="channels_last",
        interpolation="bilinear",
    )(up_conv_32)
    ...
    # DownRes 2
    conv2_32 = conv_block(
        pool2_64, FILTER_SIZE, 2 * FILTER_NUM, dropout_rate, batch_norm
    )
    pool2_16 = layers.MaxPooling2D(pool_size=(2, 2))(conv2_32)
    # DownRes 3
    conv2_16 = conv_block(
        pool2_16, FILTER_SIZE, 4 * FILTER_NUM, dropout_rate, batch_norm
    )
    ...
    # DownRes 2
    conv3_32 = conv_block(
        pool3_64, FILTER_SIZE, 2 * FILTER_NUM3, dropout_rate, batch_norm
    )
    pool3_16 = layers.MaxPooling2D(pool_size=(2, 2))(conv3_32)
    # DownRes 3
    conv3_16 = conv_block(
        pool3_16, FILTER_SIZE, 4 * FILTER_NUM3, dropout_rate, batch_norm
    )
    pool3_8 = layers.MaxPooling2D(pool_size=(2, 2))(conv3_16)
    # DownRes 4, convolution only
    conv3_8 = conv_block(
        pool3_8, FILTER_SIZE, 8 * FILTER_NUM3, dropout_rate, batch_norm
    )
    ...
    conv_final = layers.Activation("sigmoid")(
        conv_final
    )  # Change to softmax for multiclass commodity crops

    # Model integration
    model = models.Model(
        inputs=inputs, outputs=conv_final, name="Attention_UNet_Fusion"
    )
    return model


def Attention_UNetFusion3I_Sentinel(
    input_shape, NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True
):
    """
