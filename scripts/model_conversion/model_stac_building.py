...
def create_collection_with_auth(catalog_url, collection, auth):
    ...
    response = requests.post(f"{catalog_url}/collections", auth=auth, json=coll_dict)
...
def load_model_metadata(csv_path):
    ...
    if "input_channels" in df.columns:
        df["input_channels"] = df["input_channels"].apply(
            lambda x: [item.strip() for item in x.split(",")] if pd.notna(x) else []
        )
    if "output_classes" in df.columns:
        df["output_classes"] = df["output_classes"].apply(
            lambda x: [item.strip() for item in x.split(",")] if pd.notna(x) else []
        )
    if "countries_covered" in df.columns:
        df["countries_covered"] = df["countries_covered"].apply(
            lambda x: [item.strip() for item in x.split(",")] if pd.notna(x) else []
        )
    if "bbox" in df.columns:
        df["bbox"] = df["bbox"].apply(
            lambda x: (
                [float(i) for i in x.strip("[]").split(",")] if pd.notna(x) else []
            )
        )
...
def create_stac_items(df_models, collection_id):
    ...
    for _, model in df_models.iterrows():
        ...
        start_date = datetime.strptime(model["temporal_start"], "%Y-%m-%d").replace(
            tzinfo=timezone.utc
        )
        end_date = datetime.strptime(model["temporal_end"], "%Y-%m-%d").replace(
            tzinfo=timezone.utc
        )
...
def upload_items_simple(catalog_url, collection_id, items, auth):
    ...
    for item in items:
        try:
            ...
            response = requests.post(url=items_url, json=item_dict, auth=auth)
...
def main():
    ...
    try:
        ...
        auth = VitoStacApiAuthentication(username=config["username"], password=password)

        # Check if collection exists
        if not collection_exists(
            config["catalog_url"], config["collection_name"], auth
        ):
            print(
                f"Collection '{config['collection_name']}' does not exist. Creating it..."
            )
        ...
        successful, failed = upload_items_simple(
            config["catalog_url"], config["collection_name"], items, auth
        )

        if failed == 0:
            print(
                f"STAC deployment completed successfully! {successful} items uploaded."
            )
        else:
            print(
                f"STAC deployment partially completed: {successful} successful, {failed} failed"
            )
