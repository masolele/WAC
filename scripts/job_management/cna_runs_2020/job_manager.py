...
# OPENEO_AUTH_CLIENT_ID=<your_client_id_here>
...
job_dataframe = gpd.read_parquet("./job_dataframe.parquet")
logger.info("Job dataframe loaded: %d rows", len(job_dataframe))
job_database = ParquetJobDatabase("./job_database.parquet")
job_database = job_database.initialize_from_df(
    job_dataframe, on_exists="skip"
)  # Don't overwrite existing job database!
...
job_manager = MultiBackendJobManager(output_dir="./results/")
